...
router.post('/send-otp', async (req, res) => {
  try {
    ...
    if (!email.endsWith('@nie.ac.in')) {
      return res.status(400).json({ success: false, message: 'Only @nie.ac.in email addresses are allowed' });
    }
    ...
    if (error.name === 'ValidationError') {
      const firstValidationMessage = Object.values(error.errors)[0]?.message || 'Invalid email';
      return res.status(400).json({ success: false, message: firstValidationMessage });
    }
